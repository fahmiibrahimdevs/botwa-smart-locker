...
const runExpressServer = async () => {
    ...
    app.listen(PORT, HOST, () => {
        logger.info(`Whatsapp api app listening on ${HOST}:${PORT}`);
    });
    ...
    app.post("/send-media-message", body("phone_number").notEmpty().escape(), body("message").escape(), body("media").notEmpty(), body("media_filename").notEmpty(), async (req, res) => {
        // @ts-ignore
        const stateId = req.query.cred_id.toString();
        if (!waServiceClass[stateId]) {
            return res.status(400).json("connection uninitialized");
